...
def run_sequential(args, logger):
    ...
    args.n_agents = env_info["f_n_agents"]
    args.n_actions = env_info["f_n_actions"]
    ...
    f_groups = {"agents": args.n_agents}

    f_preprocess = {"actions": ("actions_onehot", [OneHot(out_dim=args.n_actions)])}

    # 跟随者智能体 buffer
    f_buffer = PrioritizedReplayBuffer(
        f_scheme,
        f_groups,
        args.buffer_size,
    ...
    # 生成多智能体控制器（mac）
    f_mac = mac_REGISTRY[args.mac](f_buffer.scheme, f_groups, args)
    ...
    f_learner = le_REGISTRY[args.learner](f_mac, f_buffer.scheme, logger, args)
    ...
    while runner.t_env <= args.t_max:
        ...
        if f_buffer.can_sample(args.batch_size):
            ...
            for _ in range(args.train_times):
                f_episode_sample, idx, weights = f_buffer.sample(
                ...
                info = f_learner.train(f_episode_sample, runner.t_env, episode, weights)
                del f_episode_sample

                # update priorities
                new_priorities = info["td_errors_abs"].flatten() + 1e-6
                f_buffer.update_priorities(idx, new_priorities.numpy().tolist())
