...
def run_sequential(args, logger):
    ...
    # 生成高reward buffer - 当做demonstrations
    ...
    # 加载 demonstration
    if args.buffer_load:
        logger.console_logger.info("Load Demonstration")
        path = f"src/MIPE-DTRL/demonstrations/{args.buffer_id}.pth"
        with open(path, "rb") as f:
            buffer_reward = pickle.load(f)
        # with open(path, "rb") as f:
    ...
    if args.model_load:
        suboptimal_agents_list = []
        folder_path = Path("record") / args.run_id / "suboptimal_agents"
        suboptimal_agents_path = [
            folder_path / item for item in args.suboptimal_agents_name
        ]

        # 加载次优专家模型
        for i in range(args.suboptimal_agents_num):
            suboptimal_agent_path = suboptimal_agents_path[i]
            suboptimal_agent = agent_REGISTRY[args.agent](mac.input_shape, args)
            # 加载模型参数
            suboptimal_agent.load_state_dict(th.load(suboptimal_agent_path))
            suboptimal_agents_list.append(suboptimal_agent)

        # 将加载的模型参数相加
        for param1, param2 in zip(
            suboptimal_agents_list[0].parameters(),
            suboptimal_agents_list[1].parameters(),
        ):
            param1.data += param2.data

        # 获得迁移后的智能体
        mac.agent = suboptimal_agents_list[0]
    ...
    while runner.t_env <= args.t_max:
        ...
        episode_batch_mean_reward = episode_batch["agent_reward"].mean()
        if episode_batch_mean_reward > 0:
            buffer_reward.insert_episode_batch(episode_batch)

        # 检查经验缓存是否足够进行采样，如果足够，就进行训练
        if args.buffer_load:
            logger.console_logger.info("Learn with Demonstrations")
            assert buffer_reward.can_sample(args.batch_size)
            if buffer.can_sample(args.batch_size):
                ...
                if np.random.rand() > args.highreward_sample:
                    # 从高reward中采样
        ...
        if (runner.t_env - last_test_T) / args.test_interval >= 1.0:
            ...
            if args.test_interval < args.max_test_interval:
                args.test_interval += args.test_interval_inc
    ...
    # 保存 demonstration
    if args.buffer_save:
        if buffer_reward.can_sample(args.buffer_size):
            logger.console_logger.info("Save Demonstrations")
            path = f"src/MIPE-DTRL/demonstrations/{os.getpid()}.pth"
            with open(path, "wb") as f:
                pickle.dump(buffer_reward, f)
        else:
            logger.console_logger.info("Demonstrations Not Full")

    if args.model_save:
        model_save_path = f"src/MIPE-DTRL/suboptimal_agents/{args.model_type}.pth"
        th.save(mac.agent.state_dict(), model_save_path)
    # if args.buffer_save:
