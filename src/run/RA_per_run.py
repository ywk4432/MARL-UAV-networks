    ...
    while runner.t_env <= args.t_max:
        ...
        if f_buffer.can_sample(f_args.batch_size):
            ...
            for _ in range(args.train_times):
                ...
                f_episode_sample, f_idx, f_weights = f_buffer.sample(
                    f_args.batch_size, runner.t_env
                )
                ...
                f_info = f_learner.train(
                    f_episode_sample, runner.t_env, episode, f_weights
                )
                del f_episode_sample
                f_info["td_errors_abs"] = f_info["td_errors_abs"].sum(dim=1)
                f_new_priorities = f_info["td_errors_abs"].flatten() + 1e-6
                f_buffer.update_priorities(f_idx, f_new_priorities.numpy().tolist())
