...
def build_n_step_td_targets(rewards, terminated, mask, target_qs, n_agents, gamma, n):
    # Assumes <target_qs > in B*T*A and <reward >, <terminated >, <mask > in (at least) B*T-1*1
    # Initialise n-step TD targets
    td_targets = target_qs.new_zeros(*target_qs.shape)

    # Calculate n-step TD targets
    for t in range(td_targets.shape[1] - n):
        future_rewards = sum([(gamma**k) * rewards[:, t + k] for k in range(n)])
        future_target_qs = (gamma**n) * target_qs[:, t + n]
        td_targets[:, t] = mask[:, t] * (
            rewards[:, t] + future_rewards + future_target_qs * (1 - terminated[:, t])
        )

    # Returns n-step TD targets from t=0 to t=T-1, i.e., in B*T-1*A
    return td_targets[:, 0:-1]
