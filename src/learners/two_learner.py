...
class Learner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            write_csv(path, [masked_td_error.abs().sum().item() / mask_elems])
        ...
        # 根据本地收益仅更新智能体更新
        self.agent_train(batch, t_env, episode_num, per_weight)
        return info

    def agent_train(
        self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None
    ):
        # Get the relevant quantities
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            # 记录各训练批次智能体的TD误差
            path = f"record/Loss/{self.args.os_pid}"
            if not os.path.exists(path):
                os.makedirs(path)
            path = path + f"/td_error_abs_agent.csv"
            write_csv(path, [masked_td_error.abs().sum().item() / mask_elems])
        ...
        if self.use_per:
            if self.return_priority:
                info["td_errors_abs_agent"] = agent_rewards.sum(1).detach().to("cpu")
                # normalize to [0, 1]
                self.priority_max = max(
                    th.max(info["td_errors_abs_agent"]).item(), self.priority_max
                )
                self.priority_min = min(
                    th.min(info["td_errors_abs_agent"]).item(), self.priority_min
                )
                info["td_errors_abs_agent"] = (
                    info["td_errors_abs_agent"] - self.priority_min
                ) / (self.priority_max - self.priority_min + 1e-5)
            else:
                info["td_errors_abs_agent"] = (
                    ((td_error.abs() * mask).sum(1) / th.sqrt(mask.sum(1)))
